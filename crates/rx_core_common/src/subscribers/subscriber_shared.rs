	...
	InError: Signal,
{
	...
	pub(crate) fn take_notification_queue(&mut self) -> Vec<SubscriberNotification<In, InError>> {
		...
		for queue_depth in 0..=SUBSCRIBER_MAX_RECURSION_DEPTH {
			let notifications = {
				...
				if queue_depth == SUBSCRIBER_MAX_RECURSION_DEPTH {
					panic!(
						"Subscriber notification queue did not drain in {SUBSCRIBER_MAX_RECURSION_DEPTH} rounds!"
					)
				}
