...
mod test {
	...
	use crate::derive_with_context_provider::impl_with_context_provider;

	#[test]
	fn should_default_to_unit() {
		let input: DeriveInput = parse_quote! { struct Foo; };
		let tokens = impl_with_context_provider(&input);
		let s = tokens.to_string();
		assert!(
			s.contains(&quote! { impl rx_core_traits::WithContextProvider for Foo }.to_string())
		);
		assert!(s.contains(&quote! { type ContextProvider = (); }.to_string()));
	}
	...
	fn should_respect_attr() {
		...
		let tokens = impl_with_context_provider(&input);
		let s = tokens.to_string();
		assert!(s.contains(&quote! { type ContextProvider = ContextType; }.to_string()));
	}
}
