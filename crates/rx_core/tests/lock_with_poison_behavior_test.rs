...
mod when_not_poisoned {
	use super::*;

	#[test]
	fn mutex_handler_is_not_called_on_clean_lock() {
		let shared = Arc::new(Mutex::new(vec!["start"]));
		let handler_called = Arc::new(AtomicBool::new(false));

		{
			let handler_called = handler_called.clone();
			let mut guard = shared.lock_with_poison_behavior(|_| {
		...
		assert!(!handler_called.load(Ordering::Relaxed));
		let guard = shared.lock().unwrap();
		assert_eq!(guard.as_slice(), ["start", "end"]);
	}

	#[test]
	fn write_handler_is_not_called_on_clean_lock() {
		let shared = Arc::new(RwLock::new(vec![1usize]));
		let handler_called = Arc::new(AtomicBool::new(false));

		{
			let handler_called = handler_called.clone();
			let mut guard = shared.write_lock_with_poison_behavior(|_| {
				handler_called.store(true, Ordering::Relaxed);
			});
			guard.push(2);
		}
	...
	#[test]
	fn read_handler_is_not_called_on_clean_lock() {
		let shared = Arc::new(RwLock::new(21usize));
		let handler_called = Arc::new(AtomicBool::new(false));
