# [operator_skip](https://github.com/AlexAegis/rx_bevy/tree/master/crates/rx_core_operator_skip)

[![crates.io](https://img.shields.io/crates/v/rx_core_operator_skip.svg)](https://crates.io/crates/rx_core_operator_skip)
[![ci](https://github.com/AlexAegis/rx_bevy/actions/workflows/ci.yml/badge.svg)](https://github.com/AlexAegis/rx_bevy/actions/workflows/ci.yml)
[![codecov](https://codecov.io/github/AlexAegis/rx_bevy/graph/badge.svg?token=hUtTGQaWMn&component=rx_core_operator_skip)](https://app.codecov.io/github/AlexAegis/rx_bevy?components%5B0%5D=rx_core_operator_skip)
[![license](https://img.shields.io/badge/license-MIT-blue.svg)](https://github.com/AlexAegis/rx_bevy?tab=MIT-1-ov-file)

Drop the first `n` values.

## See Also

- [FilterOperator](https://github.com/AlexAegis/rx_bevy/tree/master/crates/rx_core_operator_filter) -
  Keep values that satisfy a predicate.
- [FilterMapOperator](https://github.com/AlexAegis/rx_bevy/tree/master/crates/rx_core_operator_filter_map) -
  Map values to an `Option` and keep only the `Some` values.
- [TakeOperator](https://github.com/AlexAegis/rx_bevy/tree/master/crates/rx_core_operator_take) -
  Emit only the first `n` values, then complete.
- [LiftOptionOperator](https://github.com/AlexAegis/rx_bevy/tree/master/crates/rx_core_operator_lift_option) -
  Filter out `None` and forward `Some` values.

## Example

```sh
cargo run -p rx_core_operator_skip --example skip_operator_example
```

```rs
let _subscription = (1..=5)
    .into_observable()
    .skip(2)
    .subscribe(PrintObserver::new("skip_operator"));
```

Output:

```txt
skip_operator - next: 3
skip_operator - next: 4
skip_operator - next: 5
skip_operator - completed
skip_operator - unsubscribed
```
