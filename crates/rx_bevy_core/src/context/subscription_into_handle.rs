...
pub trait SubscriptionIntoScheduledHandle: ObservableSubscription + Sized + Send + Sync {
	fn into_scheduled_handle(self, context: &mut Self::Context) -> <<Self::Context as SubscriptionContext>::ScheduledSubscriptionAllocator as ScheduledSubscriptionAllocator>::ScheduledHandle<Self>;
}
...
pub trait SubscriptionIntoUnscheduledHandle: SubscriptionLike + Sized + Send + Sync {
	fn into_unscheduled_handle(self, context: &mut Self::Context) -> <<Self::Context as SubscriptionContext>::UnscheduledSubscriptionAllocator as UnscheduledSubscriptionAllocator>::UnscheduledHandle<Self>;
}
