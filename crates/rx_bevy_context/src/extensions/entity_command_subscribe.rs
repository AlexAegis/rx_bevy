...
pub trait EntityCommandSubscribeExtension {
	...
	fn subscribe_destination<Destination, S>(&mut self, destination: Destination) -> Entity
	where
	...
	#[must_use = "It is advised to save the subscriptions entity reference somewhere to be able to unsubscribe from it at will."]
	fn subscribe<Out, OutError, S>(&mut self, destination_entity: Entity) -> Entity
	where
		Out: SignalBound,
	...
	#[must_use = "It is advised to save the subscriptions entity reference somewhere to be able to unsubscribe from it at will."]
	fn subscribes_to_observable_entity<Out, OutError, S>(
		&mut self,
...
impl<'a> EntityCommandSubscribeExtension for EntityCommands<'a> {
		...
		S: ScheduleLabel,
	{
		...
		commands.subscribe::<_, S>(observable_entity, destination)
	}
