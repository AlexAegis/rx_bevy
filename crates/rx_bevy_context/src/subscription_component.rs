...
#[component(on_insert=scheduled_subscription_add_notification_observer_on_insert, on_remove=subscription_unsubscribe_on_remove)]
		...
		deferred_world.get_mut::<SubscriptionComponent>(subscription_entity)
	{
		match subscription_notification.event_mut().consume()? {
			...
			SubscriptionNotification::Add(Some(teardown)) => {
				subscription_component.add_teardown(teardown);
			}
